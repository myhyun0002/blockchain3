...
class Blockchain:
    ...
    def chain_is_valid(self, chain):
            ...
            for i in range(1, len(chain)):
                ...
                print(block_hash)
                if i != 1 and block_hash[:4] != '0000' :
                    validChain = False

                if i != 1 and prev_block['hash'] != current_block['previous_block_hash'] : #채우시오 : i가 1이 아니거나 현재 블럭의 previous_hash값과 이전 블럭의 hash값이 같은지 비교:
                    validChain = False
    ...
    def create_merkle_tree(self, transactions):
        if len(transactions) == 0:
            ...
            new_level = []
            for i in range(0, len(transactions), 2): #0부터 len(tx)까지
                left = transactions[i]
